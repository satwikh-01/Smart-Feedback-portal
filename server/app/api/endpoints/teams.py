...
def create_team(
    *,
    ...
    team = crud_team.create_team(db=db, team=team_in, manager_id=current_user.id)
    return team
...
def add_team_member(
    ...
    if user_to_add.team_id:
        raise HTTPException(status_code=400, detail="Employee is already in a team")

    return crud_team.add_employee_to_team(db=db, team=team, user=user_to_add)
